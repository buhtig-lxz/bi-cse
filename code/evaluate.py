...
class Mymteb(MTEB):
    def __init__(self, **kwargs):
        super(Mymteb, self).__init__(**kwargs)
...
def evaluate(model, args):
    model.eval()
    with torch.no_grad():
        # evaluation = Mymteb(tasks=[MyTatoeba(langs=["cmn-eng"]), MyBucc(langs=["zh-en"]), MySTSB(), MyATEC(), MyLCQMC()])
        evaluation = Mymteb(tasks=[MySTSB(),
                                   MyTatoeba(langs=["cmn-eng"]), MyBucc(langs=["zh-en"]),
                                   MyATEC(), MyLCQMC(), MyBQ(), MyPAWSX(), MyZSTSB()])
        result = evaluation.run(model, output_folder=f"result/{'student'}", overwrite_results=True, verbosity=0, eval_splits=["validation"],
...
def test(model, args):
    model.eval()
    with torch.no_grad():
        evaluation = Mymteb(tasks=[MyTatoeba(langs=["cmn-eng"]), MyBucc(langs=["zh-en"]),
                                   MySTS12(), MySTS13(), MySTS14(), MySTS15(), MySTS16(), MySTSB(), MySickR(),
                                   MySTS22(langs=['zh-en', 'en', 'zh']), MySTS17(langs=['en-en']), MyBiossesSTS(),
                                   MyATEC(), MyLCQMC(), MyAFQMC(), MyBQ(), MyPAWSX(), MyQBQTC(), MyZSTSB()])
        # evaluation = Mymteb(tasks=[MySTS22(langs=['zh-en', 'en', 'zh']), MyBiossesSTS(),
        #                            MyATEC()])
        result = evaluation.run(model, output_folder=f"result/{'student'}", overwrite_results=True, verbosity=0,
                       batch_size=args.batch_size)
